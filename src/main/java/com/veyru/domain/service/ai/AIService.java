...
public class AIService {
  ...
  private static final Map<String, List<String>> SCENE_TAGS =
      Map.of(
          "nature",
          List.of(
              "nature", "naturelovers", "beautiful", "landscape", "outdoor", "adventure", "travel"),
          "food",
          List.of("food", "foodie", "foodporn", "yummy", "delicious", "instafood", "homemade"),
          "portrait",
          List.of("portrait", "selfie", "model", "beauty", "style", "fashion", "ootd"),
          "city",
          List.of("city", "urban", "street", "citylife", "architecture", "building", "travel"),
          "beach",
          List.of("beach", "ocean", "summer", "vacation", "sun", "sea", "travel"),
          "interior",
          List.of("interior", "design", "home", "decor", "architecture", "room", "living"),
          "general",
          List.of("photo", "instagood", "photooftheday", "picoftheday", "instadaily"));
  private static final List<String> CAPTION_TEMPLATES =
      List.of(
          "Beautiful moment ✨",
          "Living my best life \ud83d\udcab",
          "Making memories \ud83d\udcf8",
  ...
  private static class UserContext {
    ...
    public double getAvgCaptionLength() {
      return avgCaptionLength;
    }

    public void setAvgCaptionLength(double avgCaptionLength) {
      this.avgCaptionLength = avgCaptionLength;
    }
  ...
  private String buildEngagementSummary(
      double avgLikes,
      double avgComments,
    ...
    summary.append(
        String.format(
            "• Trung bình %.1f lượt thích và %.1f bình luận mỗi bài\n", avgLikes, avgComments));
  ...
  public PostTimingSuggestionResponse suggestPostTiming(String userId) {
    ...
    List<Map.Entry<DayOfWeek, Double>> dayAvgs =
        engagementByDay.entrySet().stream()
            .map(
                e ->
                    Map.entry(
                        e.getKey(), e.getValue().stream().mapToDouble(d -> d).average().orElse(0)))
            .sorted(Map.Entry.<DayOfWeek, Double>comparingByValue().reversed())
            .limit(3)
            .toList();
    List<Map.Entry<Integer, Double>> hourAvgs =
        engagementByHour.entrySet().stream()
            .map(
                e ->
                    Map.entry(
                        e.getKey(), e.getValue().stream().mapToDouble(d -> d).average().orElse(0)))
  ...
  private String buildTimingSummary(
    ...
    if (!dayAvgs.isEmpty()) {
      ...
      sb.append(
          dayAvgs.get(0).getKey().getDisplayName(TextStyle.FULL, Locale.forLanguageTag("vi")));
      sb.append("\n");
    }
    if (!hourAvgs.isEmpty()) {
    ...
    sb.append(
        "\ud83d\udca1 Hãy thử nghiệm đăng bài ở các khung giờ khác nhau để tìm thời điểm phù hợp nhất!");
    return sb.toString();
  }
  ...
  private List<String> generateCaptionSuggestions(UserContext userContext) {
    ...
    if (userContext.isHasHistory() && userContext.getAvgCaptionLength() > 0) {
      if (userContext.getAvgCaptionLength() < 50) {
        ...
        suggestions.add(
            "Sharing a moment worth remembering \ud83d\udcf8\n.\n.\n#life #memories #moments");
      }
    }
    return suggestions;
  }
