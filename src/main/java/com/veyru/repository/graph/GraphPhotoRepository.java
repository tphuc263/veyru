package com.veyru.repository.graph;

import com.veyru.model.graph.GraphPhoto;
...
public interface GraphPhotoRepository extends Neo4jRepository<GraphPhoto, String> {
  ...
  List<GraphPhoto> findRecentPhotosByUser(
      @Param("userId") String userId, @Param("cutoffTime") Instant cutoffTime);
  ...
  List<GraphPhoto> findRecentPhotosByUsers(
      @Param("userIds") List<String> userIds, @Param("cutoffTime") Instant cutoffTime);
  ...
  List<GraphPhoto> findMoreFromSameUser(
      @Param("photoId") String photoId, @Param("limit") int limit);
