...
public class NewsfeedService implements INewsfeedService {
  ...
  public Page<UnifiedPostResponse> getUnifiedNewsfeed(String userId, int page, int size) {
    ...
    List<com.veyru.model.Share> shares = new ArrayList<>();
    if (!followingIds.isEmpty()) {
    ...
    // Add shares
    for (com.veyru.model.Share share : shares) {
  ...
  private Page<PhotoResponse> paginateAndConvert(
      List<Photo> photos, User currentUser, Pageable pageable) {
    int start = (int) pageable.getOffset();
