...
public class MongoIndexConfig {
  ...
  private void createPhotoIndexes() {
    ...
    ensureIndex(
        collection,
        new CompoundIndexDefinition(new Document("user.userId", 1).append("createdAt", -1)));
    ensureIndex(
  ...
  private void createLikeIndexes() {
    ...
    ensureIndex(collection, new Index().on("photoId", Sort.Direction.ASC));
    ensureIndex(
        collection,
  ...
  private void createFollowIndexes() {
    ...
    ensureIndex(collection, new Index().on("followerId", Sort.Direction.ASC));
    ensureIndex(
        collection,
